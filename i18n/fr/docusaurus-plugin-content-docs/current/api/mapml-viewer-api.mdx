...
| [controlsList](#controlslist) 	| Permet de changer l'ensemble des contrôles natifs par script. Notez l'utilisation du camelCase pour le nom de la propriété vs le nom de l'attribut en minuscules. Reproduit l'attribut `controlslist`. 	|
...
```js
let map = document.querySelector('mapml-viewer');
map.controlsList.value = "noreload nozoom"; // les valeurs peuvent être noreload | nofullscreen | nozoom | nolayer | search | geolocation
map.controlsList.add("nofullscreen"); // peut également être ajouté à l'aide de la méthode 'add'
map.controlsList.add("geolocation");  // ajoute la commande de localisation
map.controlsList.add("search"); // ajoute la commande de recherche, désactivée jusqu'à ce qu'un lien de gabarit d'URL de recherche soit défini
...
| [zoomToExtent(west, south, east, north)](#zoomtoextentwest-south-east-north) | Ajuste la vue de la carte à une étendue géographique. |
| [geojson2mapml(json, options)](#geojson2mapml) | Convertit une caractéristique GeoJSON ou une chaîne ou un objet de collection de caractéristiques en élément MapML `<map-layer>` contenant un ou plusieurs éléments `<map-feature>`. |
...
Ajuste la vue de la carte à l'étendue géographique définie par les quatre valeurs de coordonnées,
en choisissant automatiquement le niveau de zoom maximal approprié.

```js
let map = document.querySelector('mapml-viewer');
// Ajuster la carte pour afficher le sud de l'Ontario
...
| [mapsearch](#mapsearch) | Déclenché après que les réponses de recherche arrivent de toutes les couches activées pour la recherche. |
| [mapsuggestions](#mapsuggestions) | Déclenché après que les réponses de suggestions par anticipation arrivent de toutes les couches activées pour la recherche. |

### mapsearch

Déclenché sur l'élément `<mapml-viewer>` après que les réponses de recherche ont été reçues de toutes les couches cochées qui contiennent un [`<map-link rel="search">`](../elements/link/#rel). Déclenché lorsque l'utilisateur appuie sur **Entrée** dans le champ de recherche **ou sélectionne une suggestion** dans la liste déroulante d'anticipation (sélectionner une suggestion utilise le texte d'affichage de la suggestion comme requête de recherche). L'événement se propage (bubbles) et est annulable.

La structure `e.detail` est identique à [`mapsuggestions`](#mapsuggestions).

Par défaut, le gestionnaire intégré de l'événement `mapsearch` affiche une liste de résultats cliquables dans le panneau de recherche, navigue vers le premier résultat et maintient le focus sur le champ de recherche. Pour chaque entité, il s'attend à ce que `e.detail.responses[].data` soit une **FeatureCollection GeoJSON** et recherche :
...
Appelez `e.preventDefault()` pour supprimer la navigation par défaut et gérer les résultats vous-même.

```js
let map = document.querySelector('mapml-viewer');
map.addEventListener('mapsearch', (e) => {
  e.preventDefault(); // prendre en charge le traitement des résultats
  const { query, responses } = e.detail;
  for (const { data, layer } of responses) {
    // data est le JSON retourné par le point de terminaison de recherche
...
Déclenché sur l'élément `<mapml-viewer>` après que les réponses de suggestions ont été reçues de toutes les couches cochées qui contiennent un [`<map-link rel="suggestions">`](../elements/link/#rel). L'événement se propage (bubbles) et est annulable.

| Propriété `e.detail` | Description |
|---------------------|-------------|
| `query` | La chaîne de recherche saisie par l'utilisateur. |
| `responses` | Un tableau d'objets `{ data, link, layer }` — un par couche répondante. `data` est la réponse JSON analysée, `link` est l'élément `<map-link>` d'origine, et `layer` est l'élément `<map-layer>`. |

Appelez `e.preventDefault()` pour supprimer la liste déroulante de suggestions intégrée et gérer les résultats vous-même.

```js
let map = document.querySelector('mapml-viewer');
map.addEventListener('mapsuggestions', (e) => {
  e.preventDefault(); // prendre en charge le rendu
  const { query, responses } = e.detail;
  for (const { data, layer } of responses) {
    // data est le JSON retourné par le point de terminaison de suggestions
...
|           <div class="undecided">[Sélectionner la vue d'une carte à partir d'une adresse ou d'un nom de lieu (5.3.2)](https://github.com/Maps4HTML/HTML-Map-Element-UseCases-Requirements/issues/145)</div>             | complet | [complet](https://maps4html.org/web-map-doc/docs/user-guide/search) | [complet](#events) |
